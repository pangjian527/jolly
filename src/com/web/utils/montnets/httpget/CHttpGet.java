...
import com.web.utils.montnets.common.MO_PACK;
import com.web.utils.montnets.common.MULTIX_MT;
import com.web.utils.montnets.common.RPT_PACK;
import com.web.utils.montnets.common.StaticValue;
import com.web.utils.montnets.common.ValidateParamTool;
...
public class CHttpGet implements ISms {
	...
	public int SendMultixSms(StringBuffer strPtMsgId, String strUserId, String strPwd, List<MULTIX_MT> MultixMt, boolean bKeepAlive, Object connection)
	{
		...
		try {
		    ...
		    result = new SmsUtil().MongateMULTIXSendForGet(p,bKeepAlive,connection);
	...
	public int SendSms(StringBuffer strPtMsgId, String strUserId, String strPwd, String strMobiles, String strMessage, String strSubPort, String strUserMsgId, boolean bKeepAlive, Object connection)
	{
		...
		try {
			...
			result = new SmsUtil().MongateSendSubmitForGet(p,bKeepAlive,connection);
			if(result != null && !"".equals(result)&&result.length()>10){
				returnInt=0;
				strPtMsgId.append(result);
			}else if(result==null||"".equals(result)){
	...
	public int QueryBalance(StringBuffer nBalance, String strUserId, String strPwd, boolean bKeepAlive, Object connection)
	{
		...
		try{
			...
			result =  new SmsUtil().MongateQueryBalanceForGet(p,bKeepAlive,connection);
		}catch (Exception e) {
	...
	public List<MO_PACK> GetMo(String strUserId, String strPwd, boolean bKeepAlive, Object connection)
	{
		...
		try {
			...
			result =new SmsUtil().MongateGetDeliverForGet(p,bKeepAlive,connection);
	...
	@Override
	public int SendSmsNew(StringBuffer strPtMsgId, String userId,
			String password, String pszMobis, String pszMsg, String pszSubPort) {
		int returnInt=-200;
		try {
			String result = null;
			ParamsNew p = new ParamsNew();
			...
			//验证密码
			if(!ValidateParamTool.validatePwd(password)){
				strPtMsgId.append(StaticValue.PARAM_ERROR);
				return StaticValue.PARAM_ERROR;
			}
			...
			//验证手机号码,不合法就返回
			if(!ValidateParamTool.validateMobilesNew(pszMobis)){
				strPtMsgId.append(StaticValue.PARAM_ERROR);
				return StaticValue.PARAM_ERROR;
			}
			p.setPszMobis(pszMobis);
			
			//验证短信内容，不合法就返回
			if(!ValidateParamTool.validateMessageNew(pszMsg) ){
				strPtMsgId.append(StaticValue.PARAM_ERROR);
				return StaticValue.PARAM_ERROR;
			}
			p.setPszMsg(pszMsg);
			
			p.setIMobiCount(String.valueOf(pszMobis.split(",").length));
			
			//验证扩展子号是否合法，不合法就返回
			if(!ValidateParamTool.validateSubPort(pszSubPort)){
				strPtMsgId.append(StaticValue.PARAM_ERROR);
				return StaticValue.PARAM_ERROR;
			}
			p.setPszSubPort(pszSubPort);
			...
			result = new SmsUtil().MongateCsSpSendSmsNewForGet(p);
			if(result != null && !"".equals(result)&&result.length()>10){
				returnInt=0;
				strPtMsgId.append(result);
			}else if(result==null||"".equals(result)){
	...
	public int SendSmsNew(StringBuffer strPtMsgId, String userId,
			String password, String pszMobis, String pszMsg, String pszSubPort,
			String SvrType, String Param1, String Param2, String Param3,
			String Param4, String MsgId, int ModuleId, String Attime,
			String Validtime, int RptFlag) {
