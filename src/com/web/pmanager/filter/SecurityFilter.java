package com.web.pmanager.filter;
...
public class SecurityFilter implements Filter{
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse,
		...
		//如果打开登陆页面，不需要做什么处理
		String loginUrl = request.getContextPath() + "/pmanager/login/login.do";
		if (request.getRequestURI().equals(loginUrl)) {
			filterChain.doFilter(servletRequest, servletResponse);
			return;
		}
		//如果用户已经使用帐号+密码登陆了Icrm系统，可以正常访问对内的 *.jsp和*.do
		if (request.getSession().getAttribute(Consts.USER_SESSION_KEY) != null) {
			//在这里后续判断用户访问的url是否和其拥有的权限匹配，如果超出权限，则转入非法页面提示
			boolean isLegalUrl = true;
			if(isLegalUrl){
				filterChain.doFilter(servletRequest, servletResponse);
			}
			else{
