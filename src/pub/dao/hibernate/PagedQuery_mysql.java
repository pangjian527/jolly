 ...
 * Time: 4:05:51
 */
public class PagedQuery_mysql extends HibernateQuery {

	private PageSettings settings;
	private PagedQueryResult result;
	private boolean dontCount;

	public PagedQuery_mysql() {
		settings = PageSettings.of(1, 10);
		result = new PagedQueryResult();
		dontCount = false;
	}

	public PagedQuery_mysql(QuerySettings settings) {
		this();
	...
	private String constructCountSql() {
		...
		if (subQueryCount) {
			sb.append("select count(*) as ct from (");
			sb.append(getSql(false));
			sb.append(")");
		}
		else {
			sb.append("select count(*) as ct from ").append(from);
